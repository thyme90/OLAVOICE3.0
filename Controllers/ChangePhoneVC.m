...
@interface ChangePhoneVC ()<UITextFieldDelegate,MZTimerLabelDelegate>{
    UILabel *timer_show;
}
...
- (IBAction)nextStepAction:(UIButton *)sender {
    ...
    SeverStatus severStatus = [httpCon validateVcode:_oldPhoneNumber vcode:vcode];
    switch (severStatus) {
...
-(void)sendVerifyCode{
    NSLog(@"重新发送原手机号验证码");
    HttpConnect* httpCon = [HttpConnect getHttpConnectInstance];
    [httpCon getVerifyCode:_oldPhoneNumber newPhoneNum:@"" vcode:@"" opt:@"change_mobile"];
}
