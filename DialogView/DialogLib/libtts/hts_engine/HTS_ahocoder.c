...
#ifdef __cplusplus
#define HTS_AHCODER_C_START extern "C" {
#define HTS_AHCODER_C_END   }
#else
#define HTS_AHCODER_C_START
#define HTS_AHCODER_C_END
#endif                          /* __CPLUSPLUS */
...
#define DEFFV 4500.0      // si no hay modelado explícito de fvoicing, este ser?el valor de referencia
...
#define AMPNORMALIZE 0    // expandir/contraer wav hasta ocupar el rango de amplitud disponible (1) o no hacerlo (0)
...
int wavdouble2short(unsigned int Lx,double *x,double *s) {
	unsigned int k;
	for (k=0;k<Lx;k++) {
...
double fwarp(double w,double alfa) {
	...
	return atan2((1-alfa*alfa)*sin(w),(1+alfa*alfa)*cos(w)-2.0f*alfa);
}
...
int cc2waveform(double *x,unsigned int Lx,double fs,unsigned int Lframe,unsigned int Nframes,double **f0s,double **fvs,unsigned int ord,double **CC,double alfa,HTS_Audio * audio,double volume) {
	unsigned int k,kk,Kmax,K,Kuv,pm,Lp2,i;
	...
	for (k=0,pm=Lframe,f0ant=0.0;k<Nframes;k++,pm+=Lframe) {
        ...
        for(i=pm-Lframe; i<pm; i++)
        {
            short result;
            double temp = volume*x[i];
            if (temp>=0.999969482421875) result=32767;
		    else if (temp<=-1.0) result=-32768;
		    else result=(short)(32768.0*temp);

            if(HTS_Audio_write(audio,result)==0)
            {
...
int gen_ahocoder_waveform(double *s,unsigned int Ls,unsigned int sr,unsigned int Lframe,unsigned int Nframes,double **lf0s,double **fv,unsigned int ord,double alfa,double **CC,HTS_Audio * audio,double volume) {
	...
	cc2waveform((double *)s,Ls,(double)sr,Lframe,Nframes,lf0s,fv,ord,CC,alfa, audio, volume);
