...
void SegmenterManager::loadconfig(const char* confile)
{
	...
	*/
	m_config.merge_number_and_ascii = 0;
		
	m_config.seperate_number_ascii = 0;
	m_config.compress_space = 1;
...
#ifdef ANDROID_NDK_
int SegmenterManager::initDataFromBuffer(const char* pData, int len, u1 method)
{
	if( method != SEG_METHOD_NGRAM)
		return -4; //unsupport segmethod.
	...
	if(method == SEG_METHOD_NGRAM) {
		seg_freelist_.set_size(64);
		//memcpy(&buf[nLen],g_ngram_unigram_dict_name,strlen(g_ngram_unigram_dict_name));
		nRet = m_uni.loadBuffer(pData, len);
