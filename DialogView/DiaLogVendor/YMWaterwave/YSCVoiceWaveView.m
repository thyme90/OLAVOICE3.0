...
@property (nonatomic, strong) CAShapeLayer *fillShapeLyer1;
@property (nonatomic, strong) CAShapeLayer *fillShapeLyer2;
...
- (void)showInParentView:(UIView *)parentView
{
    ...
    self.frame = CGRectMake(0, parentView.bounds.size.height * 0.5, parentView.bounds.size.width, parentView.bounds.size.height * 0.5);
    [self setup];
    
    //添加两个图层，用来显示动画
    [self.layer addSublayer:self.fillShapeLyer1];
    [self.layer addSublayer:self.fillShapeLyer2];
...
- (void)updateMeters
{
    ...
    [self generateBezierPathWithFrequency1:_frequency1 maxAmplitude:_maxAmplitude phase:_phase1 lineCenter:&_lineCenter1];
    [self generateBezierPathWithFrequency2:_frequency2 maxAmplitude:_maxAmplitude*0.5 phase:_phase2 + 20 lineCenter:&_lineCenter2];
...
//{
//    CGMutablePathRef  line1Path = (__bridge CGMutablePathRef)([dic objectForKey:@"line1"]);
//    CGMutablePathRef  line2Path = (__bridge CGMutablePathRef)([dic objectForKey:@"line2"]);
//    if (line1Path && line2Path) {
//        
//        _fillShapeLyer1.path = line1Path;
//        _fillShapeLyer2.path = line2Path;
...
- (CGMutablePathRef)generateBezierPathWithFrequency1:(CGFloat)frequency maxAmplitude:(CGFloat)maxAmplitude phase:(CGFloat)phase lineCenter:(CGPoint *)lineCenter
{
    ...
    //把绘图信息添加到路径里
    CGPathAddLineToPoint(path, nil, _waveWidth, _waveHeight);
    CGPathAddLineToPoint(path, nil, 0, self.frame.size.height);
    //结束绘图信息
    CGPathCloseSubpath(path);

    _fillShapeLyer1.path = path;
    CGPathRelease(path);
    
    return path;
}

- (CGMutablePathRef)generateBezierPathWithFrequency2:(CGFloat)frequency maxAmplitude:(CGFloat)maxAmplitude phase:(CGFloat)phase lineCenter:(CGPoint *)lineCenter{
    ...
    for(CGFloat x = _beginX; x<_maxWidth; x += _density) {
        ...
        CGFloat y = scaling * maxAmplitude * normedAmplitude * _stopAnimationRatio * cosf(2 * M_PI *(x / _waveWidth) * frequency + phase) + (_waveHeight * 0.5);
...
#pragma mark - getters
- (CAShapeLayer *)fillShapeLyer1
{
    if (!_fillShapeLyer1) {
        self.fillShapeLyer1 = [CAShapeLayer layer];
        _fillShapeLyer1.lineCap = kCALineCapRound;
        _fillShapeLyer1.lineJoin = kCALineJoinRound;
        _fillShapeLyer1.fillColor = [UIColor whiteColor].CGColor;
        _fillShapeLyer1.opacity = 0.3;
        _fillShapeLyer1.frame = self.bounds;
    }
    
    return _fillShapeLyer1;
}

- (CAShapeLayer *)fillShapeLyer2
{
    if (!_fillShapeLyer2) {
        self.fillShapeLyer2 = [CAShapeLayer layer];
        _fillShapeLyer2.lineCap = kCALineCapRound;
        _fillShapeLyer2.lineJoin = kCALineJoinRound;
        _fillShapeLyer2.fillColor = [UIColor whiteColor].CGColor;
        _fillShapeLyer2.opacity = 0.3;
        _fillShapeLyer2.frame = self.bounds;
    }
    
    return _fillShapeLyer2;
}
...
- (void)updateVolume:(NSTimer *)timer
{
    ...
    //CGFloat powerEnger = [self.recorder averagePowerForChannel:0];
    float normalizedValue = [self.delegate volNum];
 //    CGFloat normalizedValue = pow (10,  normalizedValue/20);
    [self changeVolume:normalizedValue];
